    ...
    def forward(self,x):
        if not self.only_stride_1 or self.stride==1:
            if self.training:
                ...
                valid_idx = torch.arange(self.Sigma.size(0), device=self.Sigma.device)[self.Sigma != 0]
...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_classes=1000,SVD_only_stride_1 = False,decompose_type = 'channel'):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 16, n)#32x32x16
    ...
    def pruning(self):
        for m in self.modules():
            if isinstance(m,SVD_Conv2d) and m.ParamSigma is not None:
                ...
                valid_idx = torch.arange(m.ParamSigma.size(0), device=m.ParamSigma.device)[m.ParamSigma != 0]
